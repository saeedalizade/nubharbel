        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-material m-t-40" action="{{ url('admin/Post/'.$Post['id'] ) }}" name="AddPost" id="AddPost" method="POST" enctype="multipart/form-data"/>
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                    <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }}">
                        <label for="title">عنوان پست</label>
                        <input type="text" class="form-control form-control-line" id="title" name="title"  value="{{ $Post['title'] }}" />
                        @if ($errors->has('title'))
                    ...
                    <div class="form-group {{ $errors->has('type') ? ' has-error' : '' }}">
                        ...
                        <select name="status" id="status" class="form-control form-control-line select2">
                            ...
                            <option value="1" {{ $Post['status']=='1' ? 'selected' : ''}}>منتشر شده</option>
                            <option value="0" {{ $Post['status']=='0' ? 'selected' : ''}}>منتشر نشده</option>
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <input type="file" id="img" name="img" class="dropify"
                                           data-default-file="{{URL::to('img/Post/'.$Post['img'])}}"/>
                                </div>
                            </div>
                        </div>
                        @if ($errors->has('img'))
