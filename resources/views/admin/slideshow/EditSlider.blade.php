        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form class="form-material m-t-40" action="{{ url('admin/Slider/'.$Slider['id'] ) }}" name="AddSlider" id="AddSlider" method="POST" enctype="multipart/form-data"/>
                    {{ csrf_field() }}
                    {{ method_field('PATCH') }}
                    <div class="form-group {{ $errors->has('title') ? ' has-error' : '' }}">
                        <label for="title">عنوان اسلایدشو</label>
                        <input type="text" class="form-control form-control-line" id="title" name="title"  value="{{ $Slider['title'] }}" />
                        @if ($errors->has('title'))
                            ...
                            <div class="card">
                                <div class="card-body">
                                    ...
                                    <input type="file" id="img" name="img" class="dropify"
                                           data-default-file="{{URL::to('img/Slider/'.$Slider['img'])}}"/>
                                </div>
                            </div>
                        </div>
                        @if ($errors->has('img'))
